...
function runInit() {
  ...
  if (alreadyExists) {
    ...
    console.log(`${C.green}Initialized clask for ${C.bold}${PROJECT_NAME}${C.reset}`);
    ...
    console.log(`Then run: ${C.cyan}clask --now${C.reset}`);
    console.log('');
    console.log(`${C.dim}To try with sample tasks: clask --test${C.reset}`);
  }
}

const TEST_TASK_1 = `# clask Test Task 1: Create output file

**Status:** Planned — auto-generated by clask --test

## Task

Create the file \`.claude/tasks/clask-test-1.txt\` with this exact content:

\`\`\`
clask test task 1 complete
\`\`\`

## Acceptance Criteria

- [ ] File \`.claude/tasks/clask-test-1.txt\` exists
- [ ] File contains "clask test task 1 complete"
...
const TEST_TASK_2 = `# clask Test Task 2: Chain verification

**Status:** Planned — auto-generated by clask --test

## Task

Read \`.claude/tasks/clask-test-1.txt\` and confirm it contains "clask test task 1 complete".
Then create \`.claude/tasks/clask-test-2.txt\` with this exact content:

\`\`\`
clask test task 2 complete
task 1 output confirmed
\`\`\`

## Acceptance Criteria

- [ ] \`.claude/tasks/clask-test-1.txt\` exists and contains expected text (from task 1)
- [ ] \`.claude/tasks/clask-test-2.txt\` created with correct content

## Notes

Do NOT run any build or lint commands. Just read task 1 output and create task 2 file.
Depends on: clask-test-task-1.md
`;
...
async function main() {
  ...
  if (args.includes('--test')) {
    ...
    console.log('');
    console.log(`${C.bold}Now run:${C.reset}  ${C.cyan}clask --now${C.reset}  (or ${C.cyan}pnpm clask --now${C.reset})`);
    console.log(`${C.dim}Select both test tasks → confirm → watch Claude create the output files.${C.reset}`);
    console.log('');
    console.log(`${C.dim}When done, run: clask --clean-test${C.reset}`);
    return;
  }

  if (args.includes('--clean-test')) {
    const toDelete = [
      path.join(TODO_DIR, 'clask-test-task-1.md'),
      path.join(TODO_DIR, 'clask-test-task-2.md'),
      path.join(DONE_DIR, 'clask-test-task-1.md'),
      path.join(DONE_DIR, 'clask-test-task-2.md'),
      '.claude/tasks/clask-test-1.txt',
      '.claude/tasks/clask-test-2.txt',
    ];
    let removed = 0;
    toDelete.forEach(f => {
